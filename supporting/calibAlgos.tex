...
\subsubsection*{General formulation of numerical CF integration for arbitrary channel shapes}
...
\subparagraph{4}
Numerical integration for $\CF$ via trapezoidal rule:
\begin{algorithmic}
  \State \tiny \# initialize \xi near $z_0$ on the \xi-grid \normalsize
  \State $\Delta\xi \leftarrow \frac{L}{n}$
  \State $\xi < z_0$
...
\scriptsize \#Shrink both distances about half \normalsize
  \State $ \wL \leftarrow \frac{\wL + \wM}{2},\RDiff(\wL) $
  \State $ \wR \leftarrow \frac{\wR + \wM}{2},\RDiff(\wR) $
\EndIf
\EndWhile
\end{algorithmic}
\subsubsection*{S\quad subroutines}
