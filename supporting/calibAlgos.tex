...
\subsubsection*{General formulation of numerical $\boldsymbol{C_\text{F}}$ integration for arbitrary 
  channel shapes}
\vspace*{-.5ex}
The simple integration process above for three channel sections can also be generalized by the following when the 
channel 
shape is provided as a set of functions $e_s(x)$ :
...
\subparagraph{4}
Numerical integration for $\CF$ via Riemann sum (see subroutine \textbf{S1} of $\Vhyd$ above):
\begin{algorithmic}
  \State $\CF \leftarrow \text{calcCF(.....)}$
\end{algorithmic}
